...
--       무변경, 데이터 손실 없음.
-- 주의: MySQL MODIFY COLUMN은 컬럼 정의를 전체 치환하므로, 재기술하지 않은 제약(NOT NULL,
--       DEFAULT)은 무음으로 삭제된다. 원본 정의(V6)의 NOT NULL DEFAULT 0을 아래에 명시적으로
--       재기술해 제약을 보존한다(sync-phase 보안 스캔에서 발견, 정적분석 확인 후 수정).
--
...
ALTER TABLE short_sale_domestic
    MODIFY COLUMN short_sell_acc_qty_rate DECIMAL(7,4) NOT NULL DEFAULT 0
