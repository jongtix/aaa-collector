...
public record KisOverseasRightsResponse(
     ...
     * divLockDt}(→ ex_dividend_date)·{@code payDt}·{@code caTitle}(→ event_subtype)이다. 비현금배당 행에서만
     * 오는 {@code lockDt}/{@code delistDt} 등도 보존하나(원본 12필드 일치) 저장 시 매핑하지 않는다.
     */
    @JsonIgnoreProperties(ignoreUnknown = true)
    public record RightsRow(
            /** ICE공시일 (yyyyMMdd) */
            String annoDt,
            /** 권리유형 → event_subtype, 현금배당 분류 판정 키 */
            String caTitle,
