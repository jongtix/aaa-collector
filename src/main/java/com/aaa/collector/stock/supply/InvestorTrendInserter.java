...
/**
 * 투자자 매매동향 INSERT IGNORE를 단일 JDBC 커넥션 배치로 실행하고 침묵 드롭 경고를 캡처한다 (REQ-OBSV-023, AC-5).
 *
 * <p>{@code investor_trend}는 Tier-1 테이블이라 멱등 삽입에 {@code INSERT IGNORE}를 사용한다(ADR-026). INSERT
 ...
 */
// @MX:ANCHOR: [AUTO] 투자자 매매동향 INSERT IGNORE 경고 캡처 경로 — 침묵 드롭 가시화의 진입점
// @MX:REASON: SPEC-COLLECTOR-OBSV-001 REQ-OBSV-023 — InvestorTrendCollectionService가 종목별로 호출
// @MX:SPEC: SPEC-COLLECTOR-OBSV-001
@Component
@RequiredArgsConstructor
public class InvestorTrendInserter {

    // @MX:WARN: [AUTO] 권한 민감 네이티브 SQL — 반드시 INSERT IGNORE 유지 (ON DUPLICATE KEY UPDATE 금지)
    // @MX:REASON: [AUTO] collector는 investor_trend에 UPDATE 권한이 없어 ON DUPLICATE KEY UPDATE 사용 시
    ...
    /**
     * 한 종목의 검증 통과 투자자 매매동향 행들을 단일 커넥션 배치로 멱등 삽입하고 침묵 드롭 경고를 기록한다.
     *
     * <p>빈 목록이면 JDBC를 사용하지 않는다. 삽입 시도 행들의 최대 거래일로 {@code investor-trend} 워터마크를 forward-only
     * 갱신한다(SPEC-OBSV-WATERMARK-001 REQ-WM-001).
     *
     * @param rows 삽입할 검증 통과 엔티티(빈 목록이면 무동작)
     */
    public void insertBatch(List<InvestorTrend> rows) {
        if (rows.isEmpty()) {
            return;
        }
        Long drops =
        ...
        watermarkMetrics.advance(WatermarkSeries.INVESTOR_TREND, maxTradeDate(rows));
    }

    private static LocalDate maxTradeDate(List<InvestorTrend> rows) {
        return rows.stream()
    ...
    private void bindRow(PreparedStatement ps, InvestorTrend e) throws SQLException {
        ps.setObject(1, e.getStock().getId());
        ps.setObject(2, e.getTradeDate());
        ps.setLong(3, e.getForeignNetQty());
        ps.setLong(4, e.getInstitutionNetQty());
        ps.setLong(5, e.getIndividualNetQty());
        ps.setLong(6, e.getForeignNetValue());
        ps.setLong(7, e.getInstitutionNetValue());
        ps.setLong(8, e.getIndividualNetValue());
        ps.setLong(9, e.getTotalVolume());
        ps.setLong(10, e.getTotalTradingValue());
    }
}
