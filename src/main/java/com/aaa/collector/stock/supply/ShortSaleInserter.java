 ...
 */
// @MX:ANCHOR: [AUTO] 공매도 INSERT IGNORE 경고 캡처 경로 — 침묵 드롭 가시화의 진입점
// @MX:REASON: SPEC-COLLECTOR-OBSV-001 REQ-OBSV-023 — ShortSaleCollectionService가 종목별로 호출
// @MX:SPEC: SPEC-COLLECTOR-OBSV-001
@Component
@RequiredArgsConstructor
public class ShortSaleInserter {
     ...
     */
    public void insertBatch(List<ShortSaleDomestic> rows) {
        if (rows.isEmpty()) {
            return;
        }
        Long drops =
    ...
    private void bindRow(PreparedStatement ps, ShortSaleDomestic e) throws SQLException {
        ...
        ps.setLong(3, e.getShortSellQty());
        ps.setBigDecimal(4, e.getShortSellVolRate());
        ps.setLong(5, e.getShortSellAmt());
        ps.setBigDecimal(6, e.getShortSellAmtRate());
        ps.setLong(7, e.getShortSellAccQty());
