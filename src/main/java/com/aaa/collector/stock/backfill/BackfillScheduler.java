 ...
 *
 * <p>평일 02:00 KST + 주말 추가 5회(06/10/14/18/22 KST, cron {@code 6/4})로 {@link
 * BackfillOrchestrator#run()}을 발화한다. 예외를 흡수하여 스케줄러 스레드를 보호한다 — 오케스트레이터 예외가 스케줄러 스레드를 종료시키지 않는다.
 *
 * <p>주말 추가 실행 근거: WebSocket 수집이 MON-FRI 조건이라 주말에는 HikariPool 경쟁이 없어 백필 처리량을 높일 수 있다.
 *
...
public class BackfillScheduler {
     ...
     * 백필 cron 진입점.
     *
     * <p>평일/주말 공통 기본 실행: 매일 02:00 KST. 주말 추가 실행: 06/10/14/18/22 KST (기존 02:00과 합산 6회).
     *
     * <p>예외 흡수: 오케스트레이터 오류가 스케줄러 스레드를 중단시키지 않는다. 오류는 ERROR 레벨로 기록한다.
     */
    @Scheduled(cron = "${aaa.backfill.cron:0 0 2 * * *}", zone = "Asia/Seoul")
    @Scheduled(cron = "${aaa.backfill.weekend-cron:0 0 6/4 * * SAT,SUN}", zone = "Asia/Seoul")
