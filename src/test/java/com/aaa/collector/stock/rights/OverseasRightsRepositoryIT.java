 ...
 * record_dt}에 특별+정기 현금배당이 몰릴 때 {@code (stock_id, DIVIDEND, event_date)} 충돌로 두 번째 행이 silent drop되며 첫
 * 행의 {@code ex_dividend_date}가 보존되는지(acceptance 엣지· REQ-OVE-061 인지).
 *
...
@Testcontainers
@DisplayName("OverseasRightsRepository IT — 해외 현금배당 형태 멱등·동일 record_dt 충돌 silent drop")
class OverseasRightsRepositoryIT {
    ...
    @Nested
    @DisplayName("동일 record_dt 복수 현금배당 — uk 충돌 silent drop (acceptance 엣지·REQ-OVE-061)")
    class SameRecordDateCollision {

        @Test
        @DisplayName("특별+정기 현금배당이 같은 record_dt에 몰리면 두 번째 행 silent drop, 첫 행 ex_dividend_date 보존")
        void specialAndRegularSameRecordDateSecondSilentlyDropped() {
            ...
            // Act — 동일 record_dt의 특별 현금배당(다른 ex_dividend_date) — uk 충돌
            corporateEventRepository.insertIgnoreDuplicate(
                    overseasCashDividend(stock, eventDate, LocalDate.of(2026, 5, 10), "특별현금배당"));

            // Assert — INSERT IGNORE로 두 번째 행 silent drop, 첫 행 ex_dividend_date·subtype 보존
            assertThat(corporateEventRepository.countByStockId(stock.getId())).isEqualTo(1L);
            CorporateEvent saved = onlyRowOf(stock);
