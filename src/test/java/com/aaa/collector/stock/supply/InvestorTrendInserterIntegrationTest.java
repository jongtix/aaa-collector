...
@Testcontainers
@Transactional
@DisplayName("InvestorTrendInserter 통합 테스트 (실 MySQL 경고 캡처, REQ-OBSV-023/AC-5)")
@Tag("integration")
class InvestorTrendInserterIntegrationTest {
    ...
    private InvestorTrend entity(Stock stock, LocalDate date) {
        return InvestorTrend.builder()
                .stock(stock)
                .tradeDate(date)
                .foreignNetQty(1_000L)
                .institutionNetQty(-500L)
                .individualNetQty(-500L)
                .foreignNetValue(1_000_000_000L)
                .institutionNetValue(-500_000_000L)
                .individualNetValue(-500_000_000L)
                .totalVolume(2_000_000L)
                .totalTradingValue(3_000_000_000L)
                .build();
    }
    ...
    void insertsRowsWithoutSilentDrop() {
        ...
        InvestorTrendInserter inserter = buildInserter(registry);
        ...
        Integer count =
                new JdbcTemplate(dataSource)
                        .queryForObject(
                                "SELECT COUNT(*) FROM investor_trend WHERE stock_id = ?",
                                Integer.class,
                                stock.getId());
