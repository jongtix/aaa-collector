...
@DisplayName("ExtendedHoursInserter 단위 테스트 (REQ-OBSV-023 AC-5 배치 경로)")
class ExtendedHoursInserterTest {
    ...
    private ExtendedHours entity() {
        return ExtendedHours.builder()
                .stock(Stock.builder().symbol("AAPL").build())
                .session(Session.PRE)
                .tradeDate(LocalDate.of(2026, 6, 5))
                .extPrice(new BigDecimal("180.00"))
                .referenceClose(new BigDecimal("179.50"))
                .source("YAHOO")
                .collectedAt(LocalDateTime.of(2026, 6, 5, 8, 0))
                .build();
    }
