...
--       Tier-2 GRANT는 Flyway가 스키마를 만든 이후 M2의 grant 순서 훅(Tier2GrantMigrationStrategy)에서
--       DbGrantVerifier.TIER2_TABLES를 순회하며 별도 적용한다 — spec §3 D4)
--   미러 기준일: 2026-08-06
--   규칙: canonical이 항상 우선한다. aaa-infra의 두 스크립트가 변경되면 이 미러 파일을
--         canonical에 맞게 갱신한다 — 미러를 근거로 canonical을 역방향 수정하지 않는다.
--   2026-08-06 갱신(SPEC-COLLECTOR-SHORTSALE-VOLRATE-CORRECTION-001 M1): collector-tier2-grants.sql에
--   short_sale_domestic UPDATE GRANT가 추가됐다. 본 M1 파일은 변경 불요 — Tier2GrantMigrationStrategy가
--   DbGrantVerifier.TIER2_TABLES(이제 7개)를 그대로 순회하므로 short_sale_domestic도 자동으로 GRANT된다.
--
-- 프로덕션과의 차이점(의도된 것만):
