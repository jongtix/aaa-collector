...
|------|-----------------|------|---------------|
| **Tier-1** (시계열/이벤트/로그, 쓰기 전용) | `daily_ohlcv`, `domestic_news_headlines`, `overseas_news_headlines`, `macro_indicators`, `credit_balance`, `corporate_events`, `investor_trend` 등 | `SELECT, INSERT` only | **반드시 `INSERT IGNORE` 사용. `ON DUPLICATE KEY UPDATE` 절대 금지.** |
| **Tier-2** (마스터/상태, in-place 갱신 허용) | `stocks`, `stock_grades`, `short_sale_overseas`, `etf_metadata`, `backfill_status`, `market_calendar`, `short_sale_domestic` | `SELECT, INSERT, UPDATE` | UPDATE 경로 SQL 허용 |
...
2. **`DbGrantVerifier.TIER2_TABLES`** (main 소스): Tier-2 테이블 목록의 코드 단일 소스(7개, `backfill_status`·`market_calendar`·`short_sale_domestic` 포함). 테스트 유지보수 시 aaa-infra grant SQL보다 이 상수를 대조 기준으로 우선 사용한다(`DbGrantVerifierTest`가 정확히 7개를 고정).

**대조 비대상**: `Tier1InsertIgnoreGuardTest.TIER2_TABLE_ALLOWLIST`(4개)는 이 체크리스트와 무관한 별개 검사 목록이다. ADR-026 2026-06-20 개정이 `backfill_status`를 의도적으로 제외했고, `market_calendar`(SPEC-COLLECTOR-CALENDAR-001)와 `short_sale_domestic`(SPEC-COLLECTOR-SHORTSALE-VOLRATE-CORRECTION-001, ADR-026 2026-08-06 개정)도 같은 근거로 제외한다 — 가드는 `ON DUPLICATE KEY UPDATE` 패턴만 검사하는데 셋 다 그 패턴을 쓰지 않는다(시딩/자동갱신/정정 UPDATE는 `INSERT IGNORE` 또는 평이한 `UPDATE ... WHERE`). "가드 4개 vs GRANT 7개 = 표류" 진단은 오진이므로 허용목록을 "수정"하지 말 것. 상세 절차는 룰 문서(`aaa/.claude/rules/moai/development/collector-db-grant-tiers.md`)의 Sync Checklist 참조.

### GRANT 문맥(7개) vs 가드 허용목록 문맥(4개) — 서로 다른 목적의 별개 목록
...
| 목록 | 위치 | 개수 | 대상 | 목적 |
|------|------|------|------|------|
| `DbGrantVerifier.TIER2_TABLES` | `src/main/java/.../common/startup/DbGrantVerifier.java` (main 소스) | 7개: `stocks`, `stock_grades`, `short_sale_overseas`, `etf_metadata`, `backfill_status`, `market_calendar`, `short_sale_domestic` | 프로덕션 부팅 self-check가 검사하는 **실제 UPDATE GRANT 대상 테이블** | "이 테이블들에 UPDATE 권한이 실제로 부여됐는가"를 기동 시점에 검증(ADR-026 CR-01) |
| `Tier1InsertIgnoreGuardTest.TIER2_TABLE_ALLOWLIST` | `src/test/java/.../arch/Tier1InsertIgnoreGuardTest.java` (테스트 소스) | 4개: `stocks`, `stock_grades`, `short_sale_overseas`, `etf_metadata` (`backfill_status`·`market_calendar`·`short_sale_domestic` **제외**) | 정적 스캔이 `ON DUPLICATE KEY UPDATE` 리터럴 사용을 허용할 테이블 화이트리스트 | "이 SQL 패턴이 어느 테이블에 나타나면 허용/금지인가"를 빌드 타임에 정적 검사(ADR-026 결정 2) |

**차이가 나는 정확한 이유**: `backfill_status`·`market_calendar`·`short_sale_domestic`는 GRANT 목록(7개)에는 있지만 가드 허용목록(4개)에는 없다. 이들의 UPDATE 경로는 시딩(`INSERT IGNORE`)과 진행점/캘린더/정정 갱신(JPA dirty-check 또는 명시적 `UPDATE ... SET ... WHERE ...`)으로 나뉘는데, 가드가 스캔하는 패턴은 오직 `ON DUPLICATE KEY UPDATE`뿐이다 — 셋 다 이 패턴을 **애초에 쓰지 않으므로** 가드 허용목록에 없어도 오탐(false negative)이 발생하지 않는다(ADR-026 2026-06-20/2026-08-06 개정 근거, SPEC-COLLECTOR-CALENDAR-001 REQ-CAL-022가 동일 근거 재확인). 반대로 GRANT 목록은 "UPDATE 권한을 실제로 갖는가"만 따지므로 SQL 형태와 무관하게 셋 다 포함된다. 즉 4개 vs 7개는 표류(drift)가 아니라 **두 검사의 스코프가 다르기 때문에 발생하는 의도된 차이**다.
...
2. **grant 순서 훅**(M2-T2): `Tier2GrantMigrationStrategy`(`common/startup` 테스트 소스, `@Component`로 자동 감지)가 Flyway migrate 직후 root 커넥션으로 `DbGrantVerifier.TIER2_TABLES`(7개)에 UPDATE GRANT를 적용한다. 계정 미러가 없는 전용(비공유) 컨테이너에서는 `collector` 계정 자체가 없으므로 조용히 스킵한다.
